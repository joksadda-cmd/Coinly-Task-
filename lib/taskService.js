 ...
 */
export async function postTask(db, { ownerId, type, title, link, targetChatId, totalSlots, photoUrl, description }) {
...
async function notifyAdminForReview(task, ownerUser) {
  ...
  // link) — this is how the admin catches adult/inappropriate photos before
  // approving, instead of them slipping through unseen behind a text link.
  if (task.photoUrl) {
    try {
      await tgSendPhoto(ADMIN_ID, task.photoUrl, text, { reply_markup: reviewButtons });
      return;
    } catch (e) {
